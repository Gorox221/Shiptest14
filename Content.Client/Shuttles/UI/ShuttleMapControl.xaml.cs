    ...
    protected override void KeyBindUp(GUIBoundKeyEventArgs args)
    {
        if (FtlMode && ViewingMap != MapId.Nullspace)
        {
            if (args.Function == EngineKeyFunctions.UIClick)
            {
                ...
                if (TryGetBeacon(_beacons, mapTransform, args.RelativePixelPosition, PixelRect, out var foundBeacon, out _))
                {
        ...
        if (FtlMode)
        {
            if (mousePos.Window != WindowId.Invalid)
            {
                    ...
                    shuttleXform.MapID != MapId.Nullspace)
                {
                    ...
                    if (TryGetBeacon(viewportObjects, matty, mouseLocalPos, controlLocalBounds, out foundBeacon, out var foundLocalPos))
                    {
                    ...
                    var targetCoords = new EntityCoordinates(viewedMapUid, mouseMapPos);
                    var ftlFree = false;

                    if (!beaconsOnly || foundBeacon != default)
                    {
                        var beaconUid = foundBeacon != default
                            ? EntManager.GetEntity(foundBeacon.Entity)
                            : EntityUid.Invalid;

                        if (beaconUid != EntityUid.Invalid &&
                            EntManager.TryGetComponent(beaconUid, out SpaceTravelPlanetBeaconComponent? planetBeacon) &&
                            planetBeacon != null)
                        {
                            var arrivalCenter = targetCoords;
                        ...
                        {
                            ftlFree = _shuttles.FTLFree(_shuttleEntity.Value, targetCoords, _ftlAngle, _viewportExclusions);
                        }
                    }
