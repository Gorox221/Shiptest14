...
public sealed partial class NuclearReactorWindow : FancyWindow
{
    ...
    public void Update(NuclearReactorBuiState msg)
    {
        ...
        if (msg.GridWidth > 0 && msg.GridHeight > 0 &&
            (msg.GridWidth != _gridWidth || msg.GridHeight != _gridHeight || _reactorGrid.Count == 0))
        {
            _gridWidth = msg.GridWidth;
            _gridHeight = msg.GridHeight;
    ...
    protected override void FrameUpdate(FrameEventArgs args)
    {
        ...
        for (var x = 0; x < _gridWidth; x++)
        {
            ...
            {
                var vect = new Vector2i(x, y);
                if (!_data.TryGetValue(vect, out var slot))
                    continue;
                ...
                switch (_displayMode)
                {
                    case (byte)DisplayModes.Temperature:
                        box.BackgroundColor = GetColor(293.15, 1200, slot.Temperature);
                        ViewLabel.Text = Loc.GetString("comp-nuclear-reactor-ui-view-temp");
                        break;
                    case (byte)DisplayModes.Neutron:
                        box.BackgroundColor = GetColor(0, 7, slot.NeutronCount);
                ...
                _reactorRect[vect].TexturePath =
                    "/Textures/_FarHorizons/Structures/Power/Generation/FissionGenerator/reactor_part_inserted/" + slot.IconName + ".png";
                
                _reactorButton[vect].ToolTip = slot.SpentFuel > 0
                    ? "Fuel Level: " + (int)Math.Round((1 - (slot.SpentFuel / (slot.SpentFuel + (slot.Radioactivity * 0.5) + (slot.NeutronRadioactivity * 0.25)))) * 100) + "%"
                    : "";
            }
        }
