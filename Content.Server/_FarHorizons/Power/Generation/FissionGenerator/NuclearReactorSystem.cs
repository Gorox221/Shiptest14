...
public sealed class NuclearReactorSystem : SharedNuclearReactorSystem
{
    ...
    #region BUI
    private void OnUiOpened(EntityUid uid, NuclearReactorComponent comp, ref BoundUIOpenedEvent args)
    {
        UpdateUI(uid, comp);
    }
    ...
    private void EnsureGasPipes(EntityUid uid, NuclearReactorComponent comp)
    {
        ...
        var inletXform = Transform(comp.InletEnt.Value);
        if (!inletXform.Anchored)
            _transform.AnchorEntity(comp.InletEnt.Value, inletXform);

        var outletXform = Transform(comp.OutletEnt.Value);
        if (!outletXform.Anchored)
            _transform.AnchorEntity(comp.OutletEnt.Value, outletXform);
    }
    #endregion
